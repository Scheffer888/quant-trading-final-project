...
def run_intraday_hft_pipeline_with_plots(
    ...
    plt.figure()
    ax = plt.gca()
    plt.plot(best_price_df.index, mid_price, label='Mid Price', color='blue')

    # Align signals with the mid_price index using forward fill
    signals_aligned = signals_df["signals"].reindex(mid_price.index, method="ffill")
    
    # Identify change points where the signal differs from the previous time step
    change_points = signals_aligned != signals_aligned.shift()
    change_times = signals_aligned.index[change_points].to_list()

    # Ensure the final time is included to close the last segment
    if len(change_times) > 0 and change_times[-1] != mid_price.index[-1]:
        change_times.append(mid_price.index[-1])
    
    # Loop over consecutive segments and shade the background
    for i in range(len(change_times) - 1):
        start_time = change_times[i]
        end_time = change_times[i + 1]
        sig_val = signals_aligned.loc[start_time]
        # If we have multiple rows for start_time, just take the first
        ...
        ax.axvspan(start_time, end_time, color=shade_color, alpha=0.3)

    plt.title(f"{model_name}: Mid Price with Signal Background Shading")
    plt.xlabel("Time")
    plt.ylabel("Mid Price")
    plt.legend()
    plt.tight_layout()
    plt.savefig(results_dir / f"{model_name}_midprice_shaded.png")
    plt.close()
    ...
    plt.ylabel("Drawdown")
    plt.tight_layout()
    plt.savefig(results_dir / f"{model_name}_drawdown.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(results_dir / f"{model_name}_features_corr_heatmap.png")
    plt.close()
...
def single_param_sensitivity(
    ...
    for param_name, values in param_values_dict.items():
        ...
        for val in values:
            ...
            # Run the pipeline
            feats_df, signals_df, strategy_ret, final_ret = run_intraday_hft_pipeline_with_plots(
                ticker=ticker,
